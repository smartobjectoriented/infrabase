...
pr_help()
{
	...
	printf "                              given as a bare argument. A BSP recipe (e.g. bsp-linux)\n"
	printf "                              deploys the full image; a component (usr-linux, ...)\n"
	printf "                              deploys just its own part.\n"
	printf "    -v                        Emit logs during deployment\n"
	printf "Examples: \n\n"
	printf "$progname -l                  List all deployable recipes\n"
	printf "$progname usr-linux           Deploy the Linux user space into the rootfs\n"
	printf "$progname -v bsp-linux        Deploy the full Linux BSP with verbose logs\n"
}
...
# for explicitness/symmetry but is optional (`deploy.sh bsp-linux` works).
