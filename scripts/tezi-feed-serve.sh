...
do_ensure()
{
	...
	# Quietly: --ensure fires after every deploy (and after every dbuild.sh
	# command), most of which publish no feed at all — soft/hard storage
	# platforms never do. check_feed's "run a deploy first" belongs to an
	# explicit invocation, not to this automatic one.
	check_feed 2>/dev/null || return 0
