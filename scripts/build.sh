...
#
# bitbake runs as the unprivileged user. The only build invocations that
# need root (the filesystem image creation via `filesystem`, and bsp-linux
# which loop-mounts the rootfs at build time) still run bitbake
# unprivileged — the recipe calls `sudo -n` for the privileged ops
...
pr_help()
{
	...
	printf "$progname bsp-linux                       Build the full Linux BSP\n"
	printf "$progname -v -c bsp-linux                 Clean and rebuild the BSP, verbose\n\n"
	printf "On virt32/virt64, a BSP build also builds QEMU when its binary is still\n"
	printf "missing, so the tree is runnable with st.sh right away. Set\n"
	printf "IB_BUILD_QEMU=\"0\" in build/conf/local.conf to skip that.\n"
}
